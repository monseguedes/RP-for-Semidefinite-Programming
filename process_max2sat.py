...
def process_max2sat(file_name, separator="_"):
    ...
    n_vars, n_clauses = (
        file_name.split(".")[0].split(separator)[1],
        file_name.split(".")[0].split(separator)[2],
    )
...
if __name__ == "__main__":
    for i, file_name in enumerate(
        [
            ...
            if "-" not in file and ".wcnf" in file
        ]
    ):
        if 5000 <= int(file_name.split("_")[1]) <= 5000 and not os.path.exists(
            "datasets/pmax2sat/" + file_name.split(".")[0] + ".pkl"
        ):
            process_max2sat(file_name)
            print(
                f"Finished processing {file_name}, which is {i}/{len([file for file in os.listdir('datasets/MAX2SAT/')if '-' not in file and '.wcnf' in file])}"
            )
    for file_name in [
        file
        for file in os.listdir("datasets/MAX2SAT/")
        if "-" in file and ".wcnf" in file
    ]:
