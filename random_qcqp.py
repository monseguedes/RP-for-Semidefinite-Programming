...
def build_A_i(i, n):
    ...
    if i == 0:
        ...
        for i in range(math.floor(n / 2)):
            T_i[i, i] = np.random.uniform(-1 * n, 0) # -5 n
        for i in range(math.floor(n / 2) + 1, n):
            T_i[i, i] = np.random.uniform(0, 1 * n) # 5 n

    else:
        T_i = np.diag(np.random.uniform(0, 1 * n, n)) # 5 n 
...
def build_b_i(i, n):
    ...
    if i == 0:
        b_i = np.random.uniform(-1, 1, n)  # 100

    b_i = np.random.uniform(-5 * n, 0, n)  # 50
...
def random_projection_sdp(data: DataQCQP, projector, slack=True):
    ...
    with mf.Model("SDP") as M:
        ...
        if slack:
            ...
            # Lower and upper bounds of the dual variables
            epsilon = 0.00001
            dual_lower_bound = -100000 - epsilon
            dual_upper_bound = 100000 + epsilon
...
def single_problem_results(data, type="sparse", range=(0.1, 0.5), iterations=5):
    ...
    id_rp_solution = random_projection_sdp(data, id_random_projector, slack=True)
    print(
        "{: <18} {: >10} {: >8.2f} {: >8.2f}".format(
            "Identity",
...
if __name__ == "__main__":
    ...
    single_problem_results(data, type="sparser", range=(0.8, 0.9), iterations=4)
